...
At the next observation, equity is cash plus holdings marked at the current bid. Its change since the last observation includes booked fees and unrealized price changes. A change of at least +0.00002 WETH schedules a **200 ms, 20 mV-equivalent** artificial current into all **15 PAM11 (α1)** cells. A change of at most −0.00002 WETH schedules the same pulse into the **two PPL101 (γ1pedc)** cells. The pulse is binary above the threshold, not proportional to profit. The deadband is per observation; tiny changes are not accumulated into a later pulse.

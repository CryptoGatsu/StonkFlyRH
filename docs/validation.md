...
| Discovery, 18 tests | The factory topic matches Uniswap v3's `PoolCreated`; logs decode to token pair, fee and pool; only USDG-paired pools at supported tiers are candidates; a clean pool joins the universe and a failing one is rejected and remembered; the universe cap holds; scans are chunked and resume from the last block; blocklisted symbols never return; an unreadable token is a rejection, not a crash; an unheld token that stops clearing is dropped and a held one never is; the guard trades the universe, not the seed list | That the public RPC serves the log ranges asked of it, or how many pools a day chain 4663 actually creates |
